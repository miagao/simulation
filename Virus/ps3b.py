...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    sample_avg = [0 for x in range(300)]
    for trial in range(numTrials):
        ...
        for j in range(300):
            sample_avg[j] += patient.update()

        for j in range(300):
            sample_avg[j] = float(sample_avg[j])/numTrials
    
        
    pylab.plot(sample_avg)
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            protected |= not self.isResistantTo(drug)

        if not protected:    
            if random.random() < self.maxBirthProb * (1 - popDensity):
                mutant_resistances = self.resistances
                for drug in mutant_resistances:
                    if random.random() < self.getMutProb():
                        mutant_resistances[drug] = not mutant_resistances[drug]
                return ResistantVirus(self.maxBirthProb,self.clearProb, mutant_resistances, self.mutProb)
            else:
                raise NoChildException()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    population = [0 for x in range(300)]
    resistant = [0 for x in range(300)]
    for trial in range(numTrials):
        viruses = []
        for i in range(numViruses):
